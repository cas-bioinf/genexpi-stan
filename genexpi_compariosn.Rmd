...
fit_source <- fit_multiple_targets(data = data_source, model = multiple_targets_model)
...
results_orig <-  fit_multiple_targets_multi(model = multiple_targets_model, data = data_genes, output.dir = "sigB_eval")
...
for(trial in 1:num_trials) { 
  ...
  training_fits[[trial]] <- fit_multiple_targets(model = multiple_targets_model, data = training_data[[trial]]$observed)
  
  random_genes_data[[trial]] <- list()
  for(t in 1:length(candidate_genes)) {
    random_genes_data[[trial]][[t]] <- regulation_model_params(
      measurement_times = measurement_times,
      target_expression = expression_data[candidate_genes[t],],
  ...
  random_results[[trial]] <- fit_multiple_targets_multi( model = multiple_targets_model, data = random_genes_data[[trial]], output.dir = file.path("sigB_eval",paste0("random_",trial)))
...
```{r}
waic_orig <- get_waic_multiple_targets_multi(results_orig)
waic_random <- list()
for(i in 1:num_trials) {
  waic_random[[i]] <- get_waic_multiple_targets_multi(random_results[[i]])
}
save.image("genexpi_comparison.RData") 
```
