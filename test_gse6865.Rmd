...
```{r}
set.seed(45287624)
targets <- sample(rownames(expression_data[])[rowMeans(expression_data) > 1], 20)
```
...
for(t in 1:length(targets)) {
  ...
  np <- nuts_params(fit)
  n_divergent <- np %>% filter(Parameter == "divergent__") %>% get("Value",.) %>% sum()
  cat("Divergent: ", n_divergent,"\n")
