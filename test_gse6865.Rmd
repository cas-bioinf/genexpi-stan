...
```{r}
#Alternative, ignoring 1st 2 points
measurement_times = floor((gse6865_raw_time[2:14] - gse6865_raw_time[2]) / 2) + 1
smooth_time  <- seq(gse6865_raw_time[2],100, by = 2)
spline_df <- 6
spline_basis <- t(bs(smooth_time, degree = 3, df = spline_df))
num_spline_basis <- dim(spline_basis)[1]
expression_data <- gse6865_raw[,2:14]
```
...
targets <- c("codY","clpQ","yqxD","yhdL", "comA","codV","comEB", "dnaG", "ftsA","hemD","rpsS" )
...
data_source <- list(
  ...
  mean_normalized_synthesis_prior_sigma = 0.5,
...
print(summary(fit_source, pars = c("coeffs", "intercept","initial_condition","sensitivity","degradation","w","b"))$summary)

num_samples <- 100
main_geom <- geom_line(alpha = 0.2)
samples_to_show <-  sample(1:4000, num_samples)
samples_regulator <- rstan::extract(fit_source,"predicted_regulator_expression")$predicted_regulator_expression[samples_to_show,]

regulator_plot <- ggmatplot(1:data_source$num_time, t(samples_regulator), main_geom = main_geom) + ggtitle(paste0("Regulator - ", source))

if(data_source$regulator_measured != 0) {
...
#for(t in c(116,237,324,370,390,465)) {
  ...
  data <- list(
    ...
    initial_condition_prior_sigma = 2,
#    mean_normalized_synthesis_prior_sigma = 0.5,
    mean_normalized_synthesis_prior_shape = 1.1,
    mean_normalized_synthesis_prior_rate = 3,
    degradation_prior_mean = -2,
    degradation_prior_sigma = 1,
    mean_regulatory_input_prior_sigma = 5,
