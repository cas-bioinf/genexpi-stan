...
repeat {
  ...
  if(all(data$true$expression > 0)) {
    break;
  }
}
fit <- sampling(integral_model, data = data$observed, control = list(adapt_delta = 0.95))
evaluation_summary(rstan::extract(fit), data$true, printParamsResults = TRUE)
 summary(fit)$summary[,c("n_eff","Rhat")] %>% as.data.frame() %>% summarise(min_n_eff = min(n_eff),max_Rhat = max(Rhat))

num_samples <- 50
main_geom <- geom_line(alpha = 0.3)
...
```
```{r}
mcmc_parcoord(as.array(fit, pars = c("coeffs","scale","degradation","sensitivity","initial_condition")), np = nuts_params(fit)) #, np_style = parcoord_style_np(div_alpha = 1)
```
