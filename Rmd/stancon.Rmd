...
author: | 
  ...
  | martin.modrak@biomed.cas.cz, www.martinmodrak.cz
...
In the following example, we will use 5 known and 4 putative targets of the *sigA* regulator. In all of the following we use cubic spline with 6 degrees of freedom, $c_{scale} = 5$ as the basis for the main model.
...
for(t in 1:length(targets)) {
  ...
  fits_regulated[[t]] <- sampling(multiple_targets_model, data_regulated[[t]])
  waic_regulated[t] <- get_waic_multiple_targets(fits_regulated[[t]], target = 1)
}
```
