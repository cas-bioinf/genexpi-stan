...
```{r}
data <- simulate_multiple_targets_spline(3,31,5,seq(1,31,by = 3), 0.2, 0.2, regulator_measured = TRUE, integrate_ode45 = FALSE)
...
```{r}
data <- simulate_multiple_targets_spline(5,31,5,seq(1,31,by = 3), 0.1, 0.2, regulator_measured = TRUE, integrate_ode45 = FALSE)

fit <- sampling(multiple_targets_model, data = data$observed, control = list(adapt_delta = 0.95))
evaluation_summary(rstan::extract(fit, c("coeffs","initial_condition","sensitivity","degradation", "intercept","w","b","measurement_sigma_relative_param","measurement_sigma_absolute_param")), data$true, printParamsResults = TRUE)
