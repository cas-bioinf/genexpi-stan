...
fitted_regulator_plot <- function(fit, data, regulator = 1,
  ...
  if(is.null(colnames(data$regulator_expression))) {
...
fitted_target_plot <- function(fit, data, target = 1, name = "",
  ...
  if(is.null(colnames(data$expression))) {
    target_name <- target
  } else {
  ...
  ggmatplot(1:data_source$num_time, t(samples_expression[,,1]),
