...
data {
  ...
  vector<lower=0>[num_measurements] regulator_expression;
...
transformed parameters {
  ...
  {
    real min_b = -min(inv_w * regulator_profile);
    predicted_regulator_expression = regulator_profile * inv_w + min_b + b_raw;
    w = inv(inv_w);
    b = -(min_b + b_raw) / inv_w;
  }
  {
...
model {
    ...
    inv_w ~ lognormal(0,w_prior_sigma);
